...
class RemoteLogManager(fetchLog: TopicPartition => Option[UnifiedLog],
    ...
    if (predicate(segmentMetadata)) {
      ...
      remoteLogMetadataManager.updateRemoteLogSegmentMetadata(
        new RemoteLogSegmentMetadataUpdate(segmentMetadata.remoteLogSegmentId(), time.milliseconds(),
          RemoteLogSegmentState.DELETE_SEGMENT_STARTED, brokerId)).thenComposeAsync {
        new function.Function[Void, CompletableFuture[Boolean]] {
          override def apply(t: Void): CompletableFuture[Boolean] = {
            ...
            remoteLogMetadataManager.updateRemoteLogSegmentMetadata(
              new RemoteLogSegmentMetadataUpdate(segmentMetadata.remoteLogSegmentId(), time.milliseconds(),
                RemoteLogSegmentState.DELETE_SEGMENT_FINISHED, brokerId)).thenApply {
              (_: Void) => true
            }
          }
        }
                ...
                val rlsmAfterCreate = remoteLogMetadataManager.addRemoteLogSegmentMetadata(remoteLogSegmentMetadata).thenComposeAsync(
                  new function.Function[Void, CompletableFuture[RemoteLogSegmentMetadataUpdate]] {
                    override def apply(t: Void): CompletableFuture[RemoteLogSegmentMetadataUpdate] = {
                      ...
                      remoteLogMetadataManager.updateRemoteLogSegmentMetadata(rlsmAfterCreate).thenApply((_:Void) =>
                        rlsmAfterCreate
                      )
                    }
                }).get()
  ...
  def read(remoteStorageFetchInfo: RemoteStorageFetchInfo): FetchDataInfo = {
    ...
    try {
      ...
      if (remoteSegInputStream.available() > 0) Utils.drain(remoteSegInputStream, "RemoteLogSegmentInputStream")
      fetchDataInfo
    } finally {
